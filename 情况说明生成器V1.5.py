...
class SituationReportGenerator:
    ...
    def setup_data_area(self):
        ...
        self.project_entry = ttk.Entry(info_frame, textvariable=self.project_var, width=int(50 * self.scale_factor), font=("黑体", 14), style='Tall.TEntry')
        self.project_entry.grid(row=0, column=1, sticky=tk.W, padx=8, pady=5)  # 减少上下间距
        ...
        self.community_entry = ttk.Entry(info_frame, textvariable=self.community_var, width=int(50 * self.scale_factor), font=("黑体", 14), style='Tall.TEntry')
        self.community_entry.grid(row=1, column=1, sticky=tk.W, padx=8, pady=5)  # 减少上下间距
        
        # 初始化历史记录
        self.community_history = []
        self.project_history = []
        self.load_community_history()
        ...
        # 为输入框添加自动完成功能
        self.setup_autocomplete(self.project_entry, self.project_history)
        self.setup_autocomplete(self.community_entry, self.community_history, preset_communities)
    ...
    def clear_form(self, confirm=False):
        ...
        if self.apartment_frames:
            ...
            # 清空所有身份证号码
            for i in range(len(first_apt["id_card_vars"])):
                first_apt["id_card_vars"][i].set("")
    ...
    def generate_report(self):
        try:
            ...
            # 保存安置小区名称和拆迁项目名称到历史记录
            community_name = data.get("community", "")
            project_name = data.get("project", "")
            if community_name:
                self.add_to_community_history(community_name)
            if project_name:
                self.add_to_project_history(project_name)
    ...
    def load_community_history(self):
        """加载安置小区名称历史记录"""
        try:
            history_file = os.path.join(self.config_dir, "community_history.json")
            if os.path.exists(history_file):
                with open(history_file, "r", encoding="utf-8") as f:
                    import json
                    self.community_history = json.load(f)
            else:
                self.community_history = []
        except Exception as e:
            print(f"加载安置小区历史记录失败: {e}")
            self.community_history = []
    
    def load_project_history(self):
        """加载拆迁项目名称历史记录"""
        try:
            history_file = os.path.join(self.config_dir, "project_history.json")
            if os.path.exists(history_file):
                with open(history_file, "r", encoding="utf-8") as f:
                    import json
            ...
            print(f"加载拆迁项目历史记录失败: {e}")
            self.project_history = []
    
    def save_community_history(self):
        """保存安置小区名称历史记录"""
        try:
            history_file = os.path.join(self.config_dir, "community_history.json")
            with open(history_file, "w", encoding="utf-8") as f:
                import json
                json.dump(self.community_history, f, ensure_ascii=False, indent=2)
        except Exception as e:
            print(f"保存安置小区历史记录失败: {e}")
    ...
    def save_project_history(self):
        ...
        try:
            ...
            print(f"保存拆迁项目历史记录失败: {e}")
    
    def add_to_community_history(self, community_name):
        """添加到安置小区历史记录"""
        if community_name and community_name.strip():
            community_name = community_name.strip()
            # 如果已存在，先移除
            if community_name in self.community_history:
                self.community_history.remove(community_name)
            # 添加到末尾
            self.community_history.append(community_name)
            # 只保留最近的10个记录
            self.community_history = self.community_history[-10:]
            self.save_community_history()
    
    def add_to_project_history(self, project_name):
        """添加到拆迁项目历史记录"""
        if project_name and project_name.strip():
            project_name = project_name.strip()
            # 如果已存在，先移除
            if project_name in self.project_history:
                self.project_history.remove(project_name)
            # 添加到末尾
            self.project_history.append(project_name)
            # 只保留最近的10个记录
            self.project_history = self.project_history[-10:]
            self.save_project_history()
    
    def setup_autocomplete(self, entry_widget, history_list, preset_data=None):
        """为输入框设置自动完成功能"""
        # 合并历史记录和预设数据
        def get_all_data():
            all_data = list(history_list) if history_list else []
            if preset_data:
                # 将预设数据添加到前面，但避免重复
                for item in preset_data:
                    if item not in all_data:
                        all_data.insert(0, item)
            return all_data
        
        def show_suggestions(matches=None):
            """显示建议列表"""
            if matches is None:
                matches = get_all_data()[-10:]  # 显示最近的10个
            
            if matches:
                # 如果已经有自动完成列表框，先销毁
                if hasattr(entry_widget, 'autocomplete_frame'):
                    entry_widget.autocomplete_frame.destroy()
                if hasattr(entry_widget, 'autocomplete_listbox'):
                    entry_widget.autocomplete_listbox.destroy()
                
                # 创建自动完成列表框容器 - 使用root作为父容器
                root_widget = entry_widget.winfo_toplevel()
                
                # 创建外层框架用于边框和阴影效果
                entry_widget.autocomplete_frame = tk.Frame(
                    root_widget,
                    bg="#f0f0f0",
                    relief="solid",
                    borderwidth=1
                ...
                # 创建内层列表框
                entry_widget.autocomplete_listbox = tk.Listbox(
                    entry_widget.autocomplete_frame,
                ...
                # 将列表框放置在框架内
                entry_widget.autocomplete_listbox.pack(fill=tk.BOTH, expand=True, padx=1, pady=1)
                
                # 添加匹配项
                for match in matches:
                    entry_widget.autocomplete_listbox.insert(tk.END, match)
                
                # 定位外层框架 - 使用绝对坐标定位到root窗口
                x = entry_widget.winfo_rootx() - root_widget.winfo_rootx()
                y = entry_widget.winfo_rooty() + entry_widget.winfo_height() - root_widget.winfo_rooty()
                width = entry_widget.winfo_width()
                height = min(8, len(matches)) * 25 + 4  # 每行约25像素高度，加上边距
                
                entry_widget.autocomplete_frame.place(x=x, y=y, width=width, height=height)
                
                # 确保框架可见
                entry_widget.autocomplete_frame.lift()
                
                # 绑定选择事件
                def on_select(event):
                    try:
                        selection = entry_widget.autocomplete_listbox.curselection()
                        if selection:
                            selected_text = entry_widget.autocomplete_listbox.get(selection[0])
                            entry_widget.delete(0, tk.END)
                            entry_widget.insert(0, selected_text)
                            hide_suggestions()
                            entry_widget.focus_set()  # 重新获得焦点
                    except:
                        pass  # 忽略可能的错误
                
                # 保存用户原始输入内容
                if not hasattr(entry_widget, 'original_text'):
                    entry_widget.original_text = entry_widget.get()
                
                # 鼠标悬停事件 - 实时显示内容
                def on_motion(event):
                    try:
                        # 检查鼠标是否在列表框范围内
                        widget_height = entry_widget.autocomplete_listbox.winfo_height()
                        if 0 <= event.y <= widget_height:
                            # 获取鼠标位置对应的列表项
                            index = entry_widget.autocomplete_listbox.nearest(event.y)
                            if index >= 0 and index < entry_widget.autocomplete_listbox.size():
                                # 选中该项（高亮显示）
                                entry_widget.autocomplete_listbox.selection_clear(0, tk.END)
                                entry_widget.autocomplete_listbox.selection_set(index)
                                entry_widget.autocomplete_listbox.activate(index)
                                
                                # 实时显示内容到输入框
                                selected_text = entry_widget.autocomplete_listbox.get(index)
                ...
                def on_leave(event):
                    try:
                        # 恢复用户原始输入的内容
                        if hasattr(entry_widget, 'original_text'):
                            current_text = entry_widget.get()
                            if current_text != entry_widget.original_text:
                                entry_widget.delete(0, tk.END)
                                entry_widget.insert(0, entry_widget.original_text)
                        # 清除列表框选中状态
                        entry_widget.autocomplete_listbox.selection_clear(0, tk.END)
                    except:
                        pass
                ...
                # 修复事件绑定 - 改为单击选择
                entry_widget.autocomplete_listbox.bind('<Button-1>', on_select)  # 单击选择
                entry_widget.autocomplete_listbox.bind('<Return>', on_select)
                entry_widget.autocomplete_listbox.bind('<Motion>', on_motion)  # 鼠标悬停
                entry_widget.autocomplete_listbox.bind('<Leave>', on_leave)  # 鼠标离开
        
        def hide_suggestions():
            """隐藏建议列表"""
            if hasattr(entry_widget, 'autocomplete_frame'):
                entry_widget.autocomplete_frame.destroy()
                delattr(entry_widget, 'autocomplete_frame')
            if hasattr(entry_widget, 'autocomplete_listbox'):
                try:
                    entry_widget.autocomplete_listbox.destroy()
                except:
        ...
        def on_focus_in(event):
            ...
            if current_text:
                ...
                # 智能匹配：优先完全匹配开头，然后包含匹配
                exact_matches = [item for item in all_data if item.lower().startswith(current_lower)]
                partial_matches = [item for item in all_data if current_lower in item.lower() and not item.lower().startswith(current_lower)]
                
                # 合并匹配结果，完全匹配在前
                matches = exact_matches + partial_matches
                show_suggestions(matches[:10] if matches else None)
            else:
                # 如果输入框为空，显示所有历史记录
                show_suggestions()
        ...
        def on_key_release(event):
            ...
            if matches:
                show_suggestions(matches[:10])  # 最多显示10个匹配项
            else:
                hide_suggestions()
        
        def on_focus_out(event):
            # 延迟隐藏，避免点击列表框时立即隐藏
            entry_widget.after(100, hide_suggestions)
...
3. 身份证输入完成后，系统会自动验证，错误会有具体提示""")
    ...
    def on_global_click(self, event):
        ...
        # 收集所有Entry控件
        def collect_entry_widgets(widget):
            for child in widget.winfo_children():
                if isinstance(child, tk.Entry):
                    widgets_to_check.append(child)
                elif hasattr(child, 'winfo_children'):
                    collect_entry_widgets(child)
        
        collect_entry_widgets(self.root)
        
        # 检查是否点击了自动完成相关的控件
        clicked_autocomplete = False
        
        for entry_widget in widgets_to_check:
            if hasattr(entry_widget, 'autocomplete_frame') or hasattr(entry_widget, 'autocomplete_listbox'):
                # 检查是否点击了输入框本身
                if clicked_widget == entry_widget:
                    clicked_autocomplete = True
                    break
                
                # 检查是否点击了自动完成框架或列表框
                if hasattr(entry_widget, 'autocomplete_frame'):
                    try:
                        if clicked_widget == entry_widget.autocomplete_frame:
                            clicked_autocomplete = True
                            break
                    except:
                        pass
                ...
                if hasattr(entry_widget, 'autocomplete_listbox'):
                    try:
                        if clicked_widget == entry_widget.autocomplete_listbox:
                            clicked_autocomplete = True
                            break
                    except:
                        pass
        
        # 如果没有点击自动完成相关控件，则隐藏所有自动完成列表框
        if not clicked_autocomplete:
            for entry_widget in widgets_to_check:
                self.hide_autocomplete_for_widget(entry_widget)
    
    def hide_autocomplete_for_widget(self, entry_widget):
        """隐藏指定控件的自动完成列表框"""
        try:
            if hasattr(entry_widget, 'autocomplete_frame'):
                entry_widget.autocomplete_frame.destroy()
